...
class ThreeApp {
  ...
  static RENDERER_PARAM = {
    clearColor: 0x666666, // 画面をクリアする色・ 「0x」は「次にくる数は16進数ですよ」と教えてくれるマーク //@K
  ...
  constructor(wrapper) {
    ...
    // WebGLRendererクラスは描画を行う役割を担うクラス
    // WebGLRendererは画面上に映し出すためのもので、描画のための設定を行います。 //@K
    this.renderer = new THREE.WebGLRenderer()
    // クリアする色を、Colorオブジェクトで設定 //@K
    ...
    // - ジオメトリとマテリアルの初期化 ---------------------------------------
    // ジオメトリとは、3D シーン上にオブジェクトを描くために使う「頂点」の集合体です。
    // もっと言うと、ジオメトリとは「単なる形状を定義したもの」であり、
    // 言うなれば設計図、あるいは骨組みのようなものです。
    // ジオメトリはあくまでも設計図にすぎないので、これをどのように 3D 空間に配置するのかや、
    // どのような色を塗るのかは、別の概念によって決まります。
    // three.js では、どのような色を塗るのかなど質感に関する設定はマテリアルという
    // オブジェクトがそれを保持するようになっています。
    // ------------------------------------------------------------------------
    // https://ics.media/tutorial-three/geometry_general/ //@K
    this.geometry = new THREE.BoxGeometry(1.0, 1.0, 1.0) //形状（=ジオメトリ）幅・高さ・奥行 //@K
    //THREE.MeshBasicMaterialクラスはライティングを考慮しないマテリアルです。 //@K
    //陰がつかないので均一な塗りつぶした状態になります。開発中にライティングを無視して、動作確認するときに役立つでしょう。 //@K
    this.material = new THREE.MeshBasicMaterial(ThreeApp.MATERIAL_PARAM)

    // - メッシュの初期化 -----------------------------------------------------
    // three.js では、ジオメトリとマテリアルを別々に生成し組み合わせることで
    // 3D空間に配置することができるメッシュを定義できます。
    // 定義したメッシュは、シーンに追加することではじめて描画の対象になります。
    // ------------------------------------------------------------------------
    // 面を持つ三次元オブジェクトのこと
    this.box = new THREE.Mesh(this.geometry, this.material)
    // 生成したメッシュをsceneのaddメソッドを使ってシーンに追加
  ...
  render() {
    ...
    // ------------------------------------------------------------------------
    // rendererメソッドを使用して、シーンとカメラを渡して、一度のみ描画（レンダリング）する
