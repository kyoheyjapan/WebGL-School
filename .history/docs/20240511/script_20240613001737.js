// = 009 ======================================================================
// これまでのサンプルでは、メッシュは「１つのジオメトリから１つ」ずつ生成していましたが、
// 実際の案件では、同じジオメトリを再利用しながら「複数のメッシュ」を
// 生成する場面のほうが多いかもしれません。
// このとき、3D シーンに複数のオブジェクトを追加する際にやってしまいがちな間違いとして
// 「ジオメトリやマテリアルも複数回生成してしまう」というものがあります。
// メモリ効率よく複数のオブジェクトをシーンに追加する方法をしっかりおさえておきましょう。
// ============================================================================
...
class ThreeApp {
  ...
  constructor(wrapper) {
    ...
    for (let i = 0; i < torusCount; ++i) {
      ...
      // 座標をランダムに散らす
      box.position.x = (Math.pow() * 2.0 - 1.0) * transformScale
      box.position.y = (Math.pow() * 2.0 s- 1.0) * transformScale
      box.position.z = (Math.pow() * 2.0 - 1.0) * transformScale
      // シーンに追加する
      this.scene.add(box)
      // 配列に入れておく
      this.torusArray.push(box)
    }
