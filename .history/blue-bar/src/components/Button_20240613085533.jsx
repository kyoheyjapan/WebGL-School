...
const typeStyles = {
  rectangle: "group group-hover:text-txt-hov border box-border flex items-center justify-center font-bold hover:opacity-80 active:opacity-60",
  square: "group group-hover:text-txt-hov border aspect-square box-border flex items-center justify-center leading-none hover:opacity-80 active:opacity-60",
}
const sizeStyles = {
  rectangle: {
    "3xs": {
  ...
  square: {
    ...
    selector: {
      text: "typo-xs",
      icon: "h-3.5 w-3.5",
      buttonSize: "sm:min-h-10 sm:min-w-10 sm:h-10 sm:w-10 sm:max-w-10 sm:max-h-10 min-h-[39px] min-w-[39px] h-[39px] w-[39px] max-w-[39px] max-h-[39px]",
    },
    sm: {
      text: "typo-sm",
      icon: "h-4 w-4",
...
const hierarchyStyles = {
  rectangle: {
    ...
    tertiary: {
      button: "group bg-white text-txt border-bd no-underline hover:text-hov hover:bg-bg-al hover:border-bd-al active:opacity-40",
      icon: "text-icn",
    },
    transparent: {
      button: "group bg-white/0 text-txt border-none no-underline hover:text-hov group-hover:text-hov",
      icon: "text-icn group-hover:text-pri",
    },
  },
  square: {
    primary: {
      button: "bg-pri text-txt-w border-black/20 no-underline hover:opacity-50 active:opacity-30 ",
    ...
    tertiary: {
      button: "bg-white text-txt border-bd-l no-underline hover:bg-bg-al hover:opacity-50 active:opacity-30 hover:border-bd-al ",
...
export default function Button({ type = "rectangle", hierarchy = "primary", size = "xs", href = "/", onClick, iconClass, className, icon, position = "right", as = "anchor", ...props }) {
  ...
  const content = (
    <>
      {selectedIcon && position === "left" && <FontAwesomeIcon icon={selectedIcon} className={clsx(`${iconSizeClass} ${iconColorClass}`, iconClass)} />}
      <span className={textSizeClass}>{props.children}</span>
      {selectedIcon && position === "right" && <FontAwesomeIcon icon={selectedIcon} className={clsx(`${iconSizeClass} ${iconColorClass}`, iconClass)} />}
    </>
  )
